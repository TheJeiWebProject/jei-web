 ...
 *
 * References (MIT-compatible ecosystem):
...
>(graph: SankeyGraph<N, L>): void {
  ...
  for (const node of graph.nodes) {
    ...
    for (const link of coalesce(node.targetLinks, [])) {
      if (link.source !== node) incount += 1;
    }

    for (const link of coalesce(node.sourceLinks, [])) {
      if (link.target !== node) outcount += 1;
    }
  ...
  const remove = (node: SankeyNode<N, L>): void => {
    nodes.delete(node);
    for (const link of coalesce(node.targetLinks, [])) {
  ...
  const s1: SankeyNode<N, L>[] = [];
  const s2: SankeyNode<N, L>[] = [];
  while (nodes.size > 0) {
    for (;;) {
      let found = false;
    ...
    let maxNode: SankeyNode<N, L> | null = null;
    for (const node of nodes) {
      const delta =
  ...
  for (const link of graph.links) {
    ...
    if (i === j) link.direction = 'self';
    else if (i < j) link.direction = 'forward';
    else link.direction = 'backward';
  }
}

// Backward-compatible alias for older local layout implementation.
export function assignLinkDirections<
  TNode extends Record<string, unknown> = Record<string, unknown>,
  TLink extends Record<string, unknown> = Record<string, unknown>,
>(
  nodes: Array<SankeyNode<TNode, TLink>>,
  links: Array<SankeyLink<TNode, TLink>>,
): void {
  minFAS({ nodes, links });
}
